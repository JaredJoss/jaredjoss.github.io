...
The script creates a virtualenv in `cv/.venv` on first run and needs `uv` or Python >= 3.11. Edit the YAML, re-render, and commit both files — [`.github/workflows/cv.yml`](.github/workflows/cv.yml) re-renders on PRs and fails if `public/cv.pdf` is stale.
